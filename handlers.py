...
from commands import (
    start, help_command, toggle_audio, balance, reset_interactions,
    toggle_llm, toggle_voice, reset_conversation
)
from messages import handle_message
from payments import (
    pre_checkout_callback, successful_payment_callback,
    process_purchase_button, buy
)
...
def register_handlers(application):
    ...
    application.add_handler(CommandHandler("reset_conversation", reset_conversation))

    # Register message handlers
    application.add_handler(
        MessageHandler(
            filters.Regex(f"^({'|'.join(MENU_OPTIONS)})$"),
            menu_handler
        )
    )
    application.add_handler(
        MessageHandler(
            filters.TEXT & ~filters.COMMAND & ~filters.Regex(f"^({'|'.join(MENU_OPTIONS)})$"),
            handle_message
        )
    )
    ...
    application.add_handler(
        CallbackQueryHandler(process_purchase_button, pattern='^purchase_\\d+_credits$')
    )
